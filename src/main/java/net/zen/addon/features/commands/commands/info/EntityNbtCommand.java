            ...
            .then(literal("set")
                .then(argument("nbtData", StringArgumentType.greedyString())
                    .executes(context -> {
                        ...
                        try {
                            // Parse the NBT data
                            NbtCompound nbt = VersionHelper.parseNbt(nbtString);

                            // Apply the NBT data to the entity
                            VersionHelper.readEntityNbt(entity, nbt);
                ...
                .then(argument("nbtData", StringArgumentType.greedyString())
                    .executes(context -> {
                        ...
                        try {
                            ...
                            VersionHelper.writeEntityNbt(entity, currentNbt);

                            // Parse the new NBT data
                            NbtCompound newNbt = VersionHelper.parseNbt(nbtString);

                            // Merge the NBT data
                            currentNbt.copyFrom(newNbt);

                            // Apply the merged NBT data
                            VersionHelper.readEntityNbt(entity, currentNbt);
            ...
            .then(literal("get")
                .then(argument("path", StringArgumentType.word())
                    .executes(context -> {
                        ...
                        VersionHelper.writeEntityNbt(entity, nbt);
    ...
    private void displayEntityNbt(Entity entity) {
        try {
            ...
            MutableText copyButton = Text.literal("[Copy NBT]").setStyle(Style.EMPTY
                ...
                .withClickEvent(VersionHelper.createClickEvent(ClickEvent.Action.RUN_COMMAND, "/entitynbt " + entityId + " copy"))
                .withHoverEvent(VersionHelper.createShowTextHoverEvent(
